...
public class GetAdminRegion extends APIBaseData {
	...
	public void AccountCredentialWithoutToken() throws Exception {
		try {
			URL url1 = new URL("http://qa-aresapi.emersonecologics.com/admin/regions");
	...
	public void AccountCredentialWithAccount() throws InterruptedException, ClientProtocolException, IOException {
		...
		HttpGet request = new HttpGet("http://qa-aresapi.emersonecologics.com/admin/regions");
		...
		String regioncode = response.get("response_currentPage_0_regionCode");
		SoftAssertions.verifyEqualsApi(totalResults, "8000", " totalResults matches", "totalResults not matches");
		SoftAssertions.verifyEqualsApi(pageSize, "2000", " pageSize matches", "pageSize not matches");
		SoftAssertions.verifyEqualsApi(regioncode, "AA", "regioncode at 0  matches", "regioncode at 0 not matches");
		SoftAssertions.throwAsserationOnFailure();
	}
}
