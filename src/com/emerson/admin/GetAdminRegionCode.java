...
public class GetAdminRegionCode extends APIBaseData{
	...
	String actCountryCode = "US";

	@Test(priority = 1)
	public void AccountCredentialWithoutToken() throws Exception {
		try {
			URL url1 = new URL("http://qa-aresapi.emersonecologics.com/admin/regions/"+actCountryCode);
	...
	public void AccountCredentialWithAccount() throws InterruptedException, ClientProtocolException, IOException {
		...
		HttpGet request = new HttpGet("http://qa-aresapi.emersonecologics.com/admin/regions/"+actCountryCode);
		...
		String countryCode = response.get("response_currentPage_0_countryCode");
		SoftAssertions.verifyEqualsApi(totalResults, "59", " totalResults matches", "totalResults not matches");
		SoftAssertions.verifyEqualsApi(pageSize, "2000", " pageSize matches", "pageSize not matches");
		SoftAssertions.verifyEqualsApi(countryCode, actCountryCode, "countryCode at 0  matches", "countryCode at 0 not matches");
		SoftAssertions.throwAsserationOnFailure();
	}
}
