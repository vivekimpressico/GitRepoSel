...
public class PutWishlistId {
	...
	public void postAccount() {
		try {
			...
			String obj = "{\"wishListId\":\"" + actwishListId + "\",\"listItems\": [{\"itemId\":\"" + itemId1
					+ "\",\"quantity\": 0,\"note\": \"string\"}]}";
			String auth_token = APIBaseData.getUserAuthToken();
			HttpPut request = new HttpPut("http://qa-aresapi.emersonecologics.com/account/wishlists/" + actwishListId);
			...
			String wishListId = APIBaseData.dataObject.get("response_wishListId");
			String listName = APIBaseData.dataObject.get("response_listName");
			SoftAssertions.verifyEqualsApi(wishListId, actwishListId, "wishListId matches", "wishListId not matches");
			SoftAssertions.verifyEqualsApi(listName, actlistName, "listName matches", "listName not matches");
		} catch (Exception e) {
			e.printStackTrace();
